...
import {mediaDevices, MediaStream} from './components/custom/webrtc/webrtc';
...
import CustomModal from './components/agents/Modal';
...
const App: React.FC = () => {
  ...
  const playQueuedBase64 = async (base64Audio: string) => {
    ...
    try {
      const fileName = `audio_${Date.now()}.wav`; // Ensure the correct format
      ...
      // Play the audio file
      const sound = new Sound(`file://${filePath}`, '', error => {
        if (error) {
          isAudioPlaying.current = false;
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (isCapturing && !micOn.current && !videoOn && !isAudioPlaying.current) {
      ...
      <View style={styles.bottomBar}>
        ...
        <TouchableOpacity
          onPress={() => {
            ...
            showToast(
              `AEC ${
                doAEC.current ? 'enabled; only useful if you have echo issues on loudspeaker. You may experience performance issues.'
